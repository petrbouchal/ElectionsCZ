...
csvout = './CSVPS/PS_TurnoutReg.csv'

okrsky = open(csvout, 'wb')
writer = csv.writer(okrsky)
headerrow = ['Kraj', 'Okrsky', 'Volici', 'Vydane', 'Ucast', 'Odevzdane', 'Platne', 'PlatneProc', 'URL']
...
for kraj in kraje:
    ...
    if len(odevzdane) != 0:
        ...
        row = [kraj, okrskyA, voliciA, vydaneA, ucastA, odevzdaneA, platneA, platneProcA, urldat]
        writer.writerow(row)