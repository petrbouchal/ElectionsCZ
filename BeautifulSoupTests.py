...
class FollowThatTag(SoupTest):
    ...
    def testFindAllByName(self):
        ...
        self.assertEqual(matching, self.soup.find_all('a'))
        self.assertEqual(matching, self.soup.find_all(SoupStrainer('a')))

    def testFindAllByAttribute(self):
        matching = self.soup.find_all(id='x')
        self.assertEqual(len(matching), 2)
        self.assertEqual(matching[0].name, 'a')
        self.assertEqual(matching[1].name, 'b')

        matching2 = self.soup.find_all(attrs={'id' : 'x'})
        self.assertEqual(matching, matching2)

        strainer = SoupStrainer(attrs={'id' : 'x'})
        self.assertEqual(matching, self.soup.find_all(strainer))

        self.assertEqual(len(self.soup.find_all(id=None)), 1)

        self.assertEqual(len(self.soup.find_all(width=100)), 1)
        self.assertEqual(len(self.soup.find_all(junk=None)), 5)
        self.assertEqual(len(self.soup.find_all(junk=[1, None])), 5)
    ...
    def testFindAllByMethod(self):
        ...
        matching = self.soup.find_all(matchTagWhereIDMatchesName)
        self.assertEqual(len(matching), 2)
        self.assertEqual(matching[0].name, 'a')
    ...
    def testConflictingFindArguments(self):
        ...
        self.assertEqual(soup.find_all('b', text='Baz'), ['Baz'])

        self.assertEqual(soup.find(True, text='Baz'), 'Baz')
        self.assertEqual(soup.find_all(True, text='Baz'), ['Baz'])
...
class NestableEgg(SoupTest):
    ...
    def testNestedTables(self):
        ...
        self.assertEquals(len(soup.find_all('table')), 2)
