...
csvout = './CSVPS/PS_CandPrecinct2.csv'

kandidati = open(csvout, 'wb')
writer = csv.writer(kandidati)
headerrow = ['CisloObce', 'PoradiKand', 'KandJmeno', 'KandVek', 'Hlasy', 'Procenta', 'Okres', 'Kandidatka', 'Okrsek', 'URL']
...
for vesnice in okrskydata:
    ...
    if vesnice[2] > 1:
        urlvesnice = 'http://volby.cz/pls/kz2008/kz351?xjazyk=CZ&xdatum=20081017&xkraj=' + str(vesnice[3]) + '&xobec=' + vesnice[0]
        print(urlvesnice)
        try:
    ...
    for kandidatka in vsechnykandidatky:
        for okrsek in vsechnyokrsky:
            urldat = 'http://volby.cz/pls/ps2010/ps351?xjazyk=CZ&xkraj=' + vesnice[3] + '&xobec=' + vesnice[0] + '&xokrsek=' + str(okrsek) + '&xstrana=' + str(kandidatka)
            print (urldat)
            try:
            ...
            if len(poradi) != 0:
                for clovek in range(len(poradi)):
                    ...
                    if procentaA != '-':
                        row = [vesnice[3], vesnice[1].encode('utf-8','replace'), poradiA, jmenoA, vekA, hlasyA, procentaA, vesnice[4], kandidatka, okrsek, urldat]
                        writer.writerow(row)