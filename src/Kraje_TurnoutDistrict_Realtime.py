...
csvout = './CSVKraje/Kraje_TurnoutDistrict_RealTime.csv'

strany = open(csvout, 'wb')
writer = csv.writer(strany)
headerrow = ['Kraj', 'Okres', 'KrajNazev', 'OkresNazev', 'Okrsky', 'Volici', 'Vydane', 'Ucast', 'Odevzdane', 'Platne', 'PlatneProc', 'URL', 'DateTime']
writer.writerow(headerrow)

while(1 == 1):
    datumcas = datetime.strftime(datetime.now(), "%Y-%M-%DT%H:%M:%S")
