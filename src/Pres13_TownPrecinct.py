...
csvout = '../data-input/PR13_ObceOkrsky.csv'
obcecsv = open(csvout, 'wb')
writer = csv.writer(obcecsv)

writer.writerow(['CisloObce', 'NazevObce', 'PocetOkrsku', 'Kraj', 'NumNUTS', 'Okres', 'KrajName', 'URL'])

webseznam = urllib2.urlopen('http://volby.cz/pls/prez2013/pe61?xjazyk=CZ')
html = webseznam.read()
...
for bunka in range(len(bunkasurl)):
    ...
    for kus in kod:
        urldat = 'http://volby.cz/pls/prez2013/pe611?xjazyk=CZ&xnumnuts=' + kodokresu
        print urldat
        ...
        nazevkraje = krajraw[0].replace('Kraj: ', '').replace('</h3>', '').encode('utf-8', 'replace')
